...
function Profile() {
  const {
    actions: { handleFollowing, handleFollowers },
    ...
    <>
      {profileData.profile && profileTweet.tweetsById ? (
        <Wrapper>
          <Title>
            <Link to="/">
              <IconArrow size={20} icon={arrowLeft} />
            </Link>
            <h1 style={{ lineHeight: "20px" }}>
              {profileData.profile.displayName}
              <br />
              <TweetCount>
                {Object.keys(profileTweet.tweetsById).length} Tweets
              </TweetCount>
            </h1>
          </Title>
          <div>
            ...
            <InfoIcon>
              ...
              <div>
                <Icon
                  ...
                  icon={calendar}
                />{" "}
                Joined {profileData.profile.joined}
              </div>
            </InfoIcon>
                ...
                <p onClick={() => handleFollowing(profileId)}>
                  <LinkFollow to={`/${profileId}/Following`}>
                    ...
                    <span style={{ fontWeight: "700" }}>
                      {profileData.profile.numFollowing}{" "}
                    </span>
                    Following
                  </LinkFollow>
                </p>
      ...
      {profileTweet.tweetsById ? (
        <>
          ...
          >
            <Menu>
              <MenuItem style={{ borderBottom: "3px solid #2aa9e0 " }}>
                <span style={{ color: "#2aa9e0" }}>Tweets</span>
              </MenuItem>
              <MenuItem>Media</MenuItem>
              <MenuItem>Likes</MenuItem>
            </Menu>

            {Object.values(profileTweet.tweetsById)
              .map((tweet) => <TweetCard tweet={tweet} key={tweet.id} />)
              .reverse()}
          </div>
        </>
      ) : (
